...
void setEntityVisibility(int index, bool visible)
{
	if (index >= 0 && index < static_cast<int>(scenarioEntity.size()))
	{
		...
		{
			scenarioEntity[static_cast<unsigned int>(index)].entityModel->txNode_->setNodeMask(visible ? viewer::NodeMask::NODE_MASK_ENTITY_MODEL : 0x0);
			scenarioEntity[static_cast<unsigned int>(index)].visible = visible;
			if (scenarioEntity[static_cast<unsigned int>(index)].trajectory)
			{
				scenarioEntity[static_cast<unsigned int>(index)].trajectory->SetNodeMaskLines(visible ? viewer::NodeMask::NODE_MASK_TRAJECTORY_LINES : 0x0);
			}
		}
	}
}
				...
				for (int index = 0; index < static_cast<int>(scenarioEntity.size()); index++)
				{
					...
					if (index == viewer->currentCarInFocus_)
					{
						...
						snprintf(info_str_buf, sizeof(info_str_buf), "%.2fs entity[%d]: %s (%d) %.2fs %.2fkm/h %.2fm (%d, %d, %.2f, %.2f)/(%.2f, %.2f %.2f) tScale: %.2f ",
							simTime, viewer->currentCarInFocus_, state->info.name, state->info.id, state->info.timeStamp, 3.6 * static_cast<double>(state->info.speed), entry->odometer, sc->pos.roadId, // TODO: @Emil
							sc->pos.laneId, static_cast<double>(fabs(sc->pos.offset)) < SMALL_NUMBER ? 0 : static_cast<double>(sc->pos.offset), static_cast<double>(sc->pos.s), static_cast<double>(sc->pos.x), static_cast<double>(sc->pos.y), static_cast<double>(sc->pos.h), time_scale);
						viewer->SetInfoText(info_str_buf);
					}
				}
